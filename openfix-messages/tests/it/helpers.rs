...
fn test_split_message_items_std_payload() {
    assert_eq!(split_non_static!(b""), HashMap::from_iter([]));
    assert_eq!(split_non_static!(b"\x01"), HashMap::from_iter([]));
    assert_eq!(
    ...
    assert_eq!(
        ...
        HashMap::from_iter([(5, b"foo".to_vec()), (2631, b"bar".to_vec())])
    );
    assert_eq!(
...
fn test_split_message_items_weird_payload() {
    ...
    assert_eq!(split_non_static!(b"foo=bar"), HashMap::from_iter([]));
    assert_eq!(split_non_static!(b"foobar"), HashMap::from_iter([]));
}
